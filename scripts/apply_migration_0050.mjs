...
async function main() {
  ...
  const sql = `
-- RPC with fixed hash comparison and items creation
...
BEGIN
  ...
  SELECT * INTO v_scenario FROM public.pricing_scenarios
  WHERE id = p_scenario_id AND law_firm_id = v_law_firm_id FOR UPDATE;
  ...
  WHERE law_firm_id = v_law_firm_id AND actor_id = v_member_id
  AND pricing_scenario_id = p_scenario_id AND operation_type = 'create_version'
  ...
  FROM public.pricing_scenario_versions WHERE pricing_scenario_id = p_scenario_id;
...
BEGIN DELETE FROM public.pricing_idempotency_operations WHERE expires_at < now() - INTERVAL '1 hour'; END;
$$;
`;
  ...
  // Verify RPC exists
  const r = await c.query("SELECT proname FROM pg_proc WHERE proname = 'create_pricing_scenario_version_idempotent'");
  console.log('RPC exists:', r.rows.length > 0);
