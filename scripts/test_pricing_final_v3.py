...
PREFIX = f"__pricing_v3_{int(time.time() * 1000)}"
...
def dismiss_transient_overlays(page: Page) -> None:
    ...
    if consent.is_visible(timeout=1200):
        consent.click()
...
def main() -> None:
    ...
    artifact_dir = Path("artifacts/pricing-final-v3")
    ...
    stale_cleanup = cleanup_fixture_scenarios("__pricing_v3_")
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            def attach(page: Page, scope: str) -> None:
                page.on("console", lambda message: console_errors.append(f"{scope}:{message.text}") if message.type == "error" else None)
            ...
            owner_page.get_by_role("button", name=re.compile("recalcular", re.I)).click()
            owner_page.wait_for_timeout(1800)
            ...
            results["a11y"] = {
                ...
                "dialog_escape_closed": owner_page.get_by_role("dialog").count() == 0,
        ...
        try:
            cleanup_result = cleanup_fixture_scenarios("__pricing_v3_")
            remaining = rest.request("GET", "pricing_scenarios", "name=like.__pricing_v3_*&select=id")
            results["cleanup"] = {
