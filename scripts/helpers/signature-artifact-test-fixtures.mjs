    ...
    const memberRole =
      role === "suporte"
        ...
        : role === "tenant-b-owner"
          ? "proprietario"
          : role;
    const member = await admin.from("law_firm_members").insert({
      user_id: value.id,
...
export async function createSignedArtifactTestFixtures({ runId, admin }) {
  ...
  const clauses = await admin.from("contract_conversion_clauses").insert([
    {
      law_firm_id: tenantA.data.id,
      contract_id: contract.data.id,
  ...
  const document = await admin
    ...
    .insert({
      ...
      storage_path: path,
      file_name: `${runId} [artifact-fault:after_certificate].pdf`,
  ...
  const envelope = await admin
    ...
    .insert({
      ...
      contract_version_id: version.data.id,
      status: "signed",
      title: `${runId} Envelope [artifact-fault:after_certificate]`,
  ...
  const event = await admin.from("contract_signature_events").insert({
    law_firm_id: tenantA.data.id,
    envelope_id: envelopeId,
