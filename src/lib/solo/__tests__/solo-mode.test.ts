...
describe("solo navigation", () => {
  it("has exactly 3 task-oriented sections", () => {
    expect(soloNavigationSections.length).toBe(3);
  });

  it("has 'Dia a dia' as first section with core items", () => {
    const principal = soloNavigationSections[0];
    expect(principal.label).toBe("Dia a dia");
  ...
  it("does not contain enterprise-only modules", () => {
    const allHrefs = soloNavigationSections
      .flatMap((s) => s.items.flatMap((item) => [item, ...(item.children ?? [])]))
      .map((i) => i.href);
  ...
  it("keeps the first level concise and removes the duplicate office dashboard", () => {
    const topLevelItems = soloNavigationSections.flatMap((section) => section.items);
    expect(topLevelItems.length).toBeLessThanOrEqual(12);
    expect(topLevelItems.map((item) => item.href)).not.toContain("/meu-escritorio");
  });

  it("groups occasional finance and document destinations", () => {
    const topLevelItems = soloNavigationSections.flatMap((section) => section.items);
    const finance = topLevelItems.find((item) => item.href === "/recebimentos");
    const documents = topLevelItems.find((item) => item.href === "/documentos");

    expect(finance?.children?.map((item) => item.href)).toEqual(
      expect.arrayContaining(["/despesas", "/recibos", "/relatorios"]),
    );
    expect(documents?.children?.map((item) => item.href)).toContain("/documentos/modelos");
  });

  it("keeps the grouped destination available at the parent level", () => {
    const topLevelItems = soloNavigationSections.flatMap((section) => section.items);
    const groupedDestinations = [
      ["Novos atendimentos", "/atendimentos"],
      ["Financeiro", "/recebimentos"],
      ["Documentos", "/documentos"],
    ] as const;

    for (const [label, href] of groupedDestinations) {
      expect(topLevelItems.find((item) => item.label === label)?.href).toBe(href);
    }
  });
