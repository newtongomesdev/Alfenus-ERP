...
describe("calculateInstallmentPlan", () => {
  ...
  it("mantém a entrada como recebível explícito quando solicitado", () => {
    const installments = calculateInstallmentPlan({
      totalAmountCents: 100_00,
      upfrontAmountCents: 25_00,
      installments: 4,
      includeUpfrontAsReceivable: true,
    });

    expect(installments[0]).toEqual({ number: 0, amountCents: 25_00 });
    expect(installments.reduce((total, item) => total + item.amountCents, 0)).toBe(100_00);
  });
});
