...
type IncidentQueryResult = { data: IncidentRow[] | null; count: number | null };
type IncidentQuery = {
  select: (...args: unknown[]) => IncidentQuery;
  eq: (column: string, value: string) => IncidentQuery;
  ilike: (column: string, value: string) => IncidentQuery;
  order: (column: string, options: { ascending: boolean }) => IncidentQuery;
  range: (from: number, to: number) => Promise<IncidentQueryResult>;
  then: Promise<IncidentQueryResult>["then"];
};
type IncidentAdminClient = { from: (table: string) => IncidentQuery };
...
export default async function AdminSecurityIncidentsPage({
  ...
  // This server-side try/catch protects data loading; React error boundaries cannot catch the awaited query.

  try {
    // Fetch all incidents for metrics
    const { data: allIncidents } = await (adminClient as unknown as IncidentAdminClient)
      .from("security_incidents")
      .select("id, severity, status, created_at, law_firm_id");

    const totalCount = allIncidents?.length ?? 0;
    const abertosCount = allIncidents?.filter((i) => i.status === "aberto").length ?? 0;
    const altaCriticaCount = allIncidents?.filter((i) => i.severity === "alta" || i.severity === "critica").length ?? 0;

    const now = new Date();
    const mesAtual = now.getMonth();
    const anoAtual = now.getFullYear();
    const resolvidosEsteMesCount = allIncidents?.filter((i) => {
    ...
    let query = (adminClient as unknown as IncidentAdminClient)
      .from("security_incidents")
