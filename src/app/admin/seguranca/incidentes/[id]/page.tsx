...
type IncidentDetailQuery = {
  select: (...args: unknown[]) => IncidentDetailQuery;
  eq: (column: string, value: string) => IncidentDetailQuery;
  single: () => Promise<{ data: IncidentDetail | null }>;
  order: (column: string, options: { ascending: boolean }) => Promise<{ data: IncidentEvent[] | null }>;
};
type IncidentDetailClient = { from: (table: string) => IncidentDetailQuery };
...
export default async function AdminSecurityIncidentDetailPage({
  ...
  try {
    const { data: incident } = await (adminClient as unknown as IncidentDetailClient)
    ...
    const { data: events } = await (adminClient as unknown as IncidentDetailClient)
