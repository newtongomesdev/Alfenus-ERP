...
export default async function DeadlinesPage({ searchParams }: { searchParams: Promise<{ criado?: string; concluido?: string; erro?: string; page?: string }> }) {
  ...
  let overview: Awaited<ReturnType<typeof getDeadlinesOverview>> | null = null;
  let options: Awaited<ReturnType<typeof getDeadlineOptions>> | null = null;

  try {
    context = await getAppContext();
    if (context.status !== "ready" || !context.member || !context.lawFirm) unavailableStatus = context.status;
    else [overview, options] = await Promise.all([getDeadlinesOverview(context.lawFirm.id, page, PAGE_SIZE), getDeadlineOptions(context.lawFirm.id)]);
  } catch {
    return (
  ...
  if (unavailableStatus || !context.member || !context.lawFirm || !overview || !options) {
    return <Unavailable status={unavailableStatus ?? context.status} />;
  }
  ...
  return <AppShell memberName={context.member.name}><div className="space-y-6">
    <PageHeader title="Prazos" description="Controle datas limite, prioridades e conclusão das atividades jurídicas." actions={<div className="flex flex-wrap gap-2"><Link href="/prazos/calculos" className="inline-flex h-8 items-center justify-center rounded-lg border border-border px-3 text-sm font-medium hover:bg-muted">Cálculo de prazos</Link><Link href="/prazos/calendarios" className="inline-flex h-8 items-center justify-center rounded-lg border border-border px-3 text-sm font-medium hover:bg-muted">Calendários</Link>{canCreate ? <a href="#novo-prazo" className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg bg-primary px-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/80"><Plus className="size-4" /> Novo prazo</a> : null}</div>} />
