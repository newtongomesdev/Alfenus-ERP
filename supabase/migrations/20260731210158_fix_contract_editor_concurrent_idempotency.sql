-- Reuse a completed operation before evaluating the caller's stale timestamp.
-- This makes concurrent retries deterministic while retaining the row lock for
-- genuinely new versions.
create or replace function public.create_contract_version(
  p_contract_id uuid,p_expected_updated_at timestamptz,p_title text,p_content text,p_parties jsonb,p_clauses jsonb,p_terms jsonb,p_metadata jsonb,p_content_hash text,p_idempotency_key text,p_input_hash text,p_activate boolean default false
) returns table(contract_version_id uuid, version_number integer, idempotent boolean, updated_at timestamptz)
language plpgsql security definer set search_path=public,extensions as $$
declare v_firm uuid; v_member uuid; v_contract public.contracts%rowtype; v_existing public.contract_version_operations%rowtype; v_version_id uuid; v_number integer; v_clause jsonb; v_index integer:=0; v_now timestamptz:=now();
begin
 ...
 if v_existing.id is not null then
   ...
   return query select v_existing.contract_version_id,cv.version_number,true,c.contract_editor_updated_at from public.contracts c join public.contract_conversion_versions cv on cv.id=v_existing.contract_version_id where c.id=p_contract_id;
   return;
 end if;
 ...
 if v_existing.id is not null then
   if v_existing.input_hash<>p_input_hash then raise exception 'CONTRACT_EDITOR_IDEMPOTENCY_CONFLICT' using errcode='23505'; end if;
   return query select v_existing.contract_version_id,cv.version_number,true,v_contract.contract_editor_updated_at from public.contract_conversion_versions cv where cv.id=v_existing.contract_version_id;
   return;
 end if;
