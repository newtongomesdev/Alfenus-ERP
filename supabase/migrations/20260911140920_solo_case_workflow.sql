-- Creates the streamlined Solo workflow in one database transaction.
-- The function deliberately derives the tenant and acting member from auth.uid().
...
begin
  ...
  select id, law_firm_id, role
    into v_member_id, v_law_firm_id, v_role
  ...
  if p_existing_client_id is not null then
    ...
    insert into public.clients (
      law_firm_id, name, phone, whatsapp, email, document, interest_area,
      responsible_member_id, status
    ) values (
      v_law_firm_id, trim(p_client_name), nullif(trim(p_client_phone), ''),
      nullif(trim(p_client_phone), ''), nullif(trim(p_client_email), ''),
      nullif(trim(p_client_document), ''), nullif(trim(p_client_interest_area), ''),
      v_member_id, 'ativo'
  ...
  insert into public.legal_cases (
    law_firm_id, client_id, title, case_kind, action_type, case_number,
    main_responsible_id, status, priority, opposing_party, strategic_notes
  ) values (
    v_law_firm_id, v_client_id, trim(p_case_title), p_case_kind,
    trim(p_action_type), nullif(trim(p_case_number), ''), v_member_id,
    'em_analise', 'normal', nullif(trim(p_opposing_party), ''), nullif(trim(p_notes), '')
  ...
  if p_contract_total_amount_cents is not null or p_contract_service_description is not null then
    if p_contract_total_amount_cents is null
      or p_contract_total_amount_cents <= 0
      or length(trim(coalesce(p_contract_service_description, ''))) < 5
      or p_contract_first_due_date is null
      or length(trim(coalesce(p_contract_payment_method, ''))) < 2
      or p_contract_upfront_amount_cents < 0
      or p_contract_upfront_amount_cents > p_contract_total_amount_cents
      or p_contract_installments_count < 1 then
    ...
    insert into public.contracts (
      law_firm_id, client_id, legal_case_id, service_description,
      total_amount_cents, upfront_amount_cents, balance_cents, has_installments,
      installments_count, first_due_date, frequency, payment_method,
      responsible_member_id, status
    ) values (
      v_law_firm_id, v_client_id, v_case_id, trim(p_contract_service_description),
      p_contract_total_amount_cents, p_contract_upfront_amount_cents, v_balance_cents,
      p_contract_installments_count > 1, p_contract_installments_count,
      p_contract_first_due_date, 'mensal', trim(p_contract_payment_method),
      v_member_id, 'ativo'
    ...
    for v_index in 1..p_contract_installments_count loop
      ...
      insert into public.installments (
        law_firm_id, contract_id, client_id, number, original_amount_cents,
        final_amount_cents, due_date, payment_method, status
      ) values (
        v_law_firm_id, v_contract_id, v_client_id, v_index,
        v_installment_cents + case when v_index <= v_remainder then 1 else 0 end,
        v_installment_cents + case when v_index <= v_remainder then 1 else 0 end,
        v_due_date, trim(p_contract_payment_method),
        case when v_installment_cents = 0 and v_remainder = 0 then 'pago' else 'pendente' end
      );
    end loop;

    insert into public.audit_logs (law_firm_id, actor_id, action, entity_type, entity_id, metadata)
    values (v_law_firm_id, v_member_id, 'criou_contrato', 'contract', v_contract_id, jsonb_build_object('origem', 'novo_caso_solo', 'legal_case_id', v_case_id));
  end if;
  ...
  if p_deadline_title is not null or p_deadline_date is not null then
    ...
    insert into public.deadlines (
      law_firm_id, title, type, client_id, legal_case_id, responsible_member_id,
      due_date, priority, status
    ) values (
      v_law_firm_id, trim(p_deadline_title), 'processual', v_client_id, v_case_id,
      v_member_id, p_deadline_date, p_deadline_priority, 'pendente'
...
revoke all on function public.create_solo_case(
  uuid, text, text, text, text, text, text, text, text, text, text, text,
  text, bigint, bigint, integer, date, text, text, date, public.priority_level
) from public;

grant execute on function public.create_solo_case(
  uuid, text, text, text, text, text, text, text, text, text, text, text,
  text, bigint, bigint, integer, date, text, text, date, public.priority_level
) to authenticated;

drop policy if exists "financial roles write contracts" on public.contracts;
create policy "contract managers write contracts"
on public.contracts for all to authenticated
using (public.has_law_firm_role(law_firm_id, array['proprietario', 'administrador', 'advogado', 'financeiro']::public.member_role[]))
with check (public.has_law_firm_role(law_firm_id, array['proprietario', 'administrador', 'advogado', 'financeiro']::public.member_role[]));

drop policy if exists "financial roles write installments" on public.installments;
create policy "contract managers write installments"
